        ...
        devShells = rec {
          ...
          docker-build = pkgs.mkShell {
            ...
            shellHook = ''
              ...
              PODMAN_SOCKET_DIR=$(find /tmp/nix-shell.*/podman -name "podman-machine-default-api.sock" -type s 2>/dev/null | head -n 1 | xargs dirname)
              if [ -n "$PODMAN_SOCKET_DIR" ]; then
                export DOCKER_HOST="unix://$PODMAN_SOCKET_DIR/podman-machine-default-api.sock"
